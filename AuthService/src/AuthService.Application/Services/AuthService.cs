    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    private UserResponseDto MapToUserResponseDto(User user)
    {
        return new UserResponseDto
        {
            ...
            Email = user.Email,
            ProfilePhoto = _cloudinaryService.GetFullImageUrl(user.UserProfile?.ProfilePhoto ?? string.Empty),
            Phone = user.UserProfile?.Phone ?? string.Empty,
            Role = user.UserRoles?.FirstOrDefault()?.Role?.Name ?? RoleConstants.ADMIN_ROLE,
            Status = user.Status,
